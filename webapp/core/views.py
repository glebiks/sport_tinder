...
def index(request):
    ...
    print(user_matches)
    user_suggestions = Profile.objects.filter(~Q(user=request.user)) # также сделать проверку на тех, кто уже есть в списке mathces
                                                                     # or Profile.id_user not in user_matches
